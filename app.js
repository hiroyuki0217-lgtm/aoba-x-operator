...
const contentAnalysis = {
  life: { goal:'共感・プロフィール遷移', hook:'バッグ、靴、冷蔵庫など一つの生活物を先に置く。', visual:'顔よりも「帰宅した直後」が分かる画を選ぶ。', test:'同じ場面で、人物あり／手元だけを1回ずつ比べる。', avoid:'疲労を大げさにしない。説明を足しすぎない。' },
  realestate: { goal:'保存的な反応・信頼', hook:'専門用語ではなく、内見中に感じる小さな違和感や発見から入る。', visual:'窓、収納、玄関など本文で触れた一点を写す。', test:'知識を1点だけに絞り、疑問形と独り言形を比べる。', avoid:'物件名・所在地・顧客・断定的な助言を書かない。' },
  room: { goal:'画像の停止率・プロフィール遷移', hook:'暮らしが想像できる動線を一つだけ言葉にする。', visual:'広角の全景より、玄関からの視線や窓辺など入口になる構図。', test:'人物の後ろ姿あり／部屋のみを別日に1回ずつ比べる。', avoid:'実在の物件と誤認させる住所・家賃・募集情報を書かない。' },
  tired: { goal:'共感・短い返信', hook:'「できなかったこと」ではなく、最後にできた小さなことを置く。', visual:'パンプス、歯ブラシ、飲みかけの水など回復に向かう物を選ぶ。', test:'弱音の強さは一度に上げず、平常文との反応差だけを見る。', avoid:'自傷・絶望・孤立を連想させる表現や画像にはしない。' },
  question: { goal:'返信・会話の入口', hook:'答えやすく、誰かを試さない一問だけにする。', visual:'質問と関係する日常物を一つ添えて、広告っぽさを消す。', test:'「ありますか」と「どうしてますか」を別日に1回ずつ比べる。', avoid:'毎回質問にしない。返答の催促や正解探しをしない。' },
  support: { goal:'やさしい返信・関係性', hook:'助けを求めるより、「一言があれば嬉しい」程度に留める。', visual:'朝の準備、資料、腕時計など、次へ進む感じが残る画。', test:'応援を求める文と、自分で締める文を別日に1回ずつ比べる。', avoid:'読者に責任を負わせる言い方、継続的な慰めの要求にしない。' }
};
const dailyMoments = {
  weekday: [
...
function posts(){ const plan=planForToday(), themeId=plan.theme||state.dailyTheme, insight=contentAnalysis[themeId]||contentAnalysis.life; return header('今日の投稿設計','投稿の種類を選ぶと、あおばの話し方に合わせた下書きと画像案に切り替わります。公開はあなたの最終確認後です。')+`<section class="card theme-picker"><p class="eyebrow">今日は何を話す？</p><div class="theme-chips"><button class="theme-chip ${state.dailyTheme==='auto'?'selected':''}" data-theme="auto">おまかせ</button>${contentThemes.map(theme=>`<button class="theme-chip ${state.dailyTheme===theme.id?'selected':''}" data-theme="${theme.id}">${esc(theme.name)}</button>`).join('')}</div><p class="hint">${state.dailyTheme==='auto'?'曜日と時間に合わせた案です。':'「'+esc(contentThemes.find(theme=>theme.id===state.dailyTheme)?.hint||'')+'」'}</p></section><section class="card daily-plan"><p class="eyebrow">${plan.dateLabel} · ${plan.time}ごろ · ${plan.label}</p><p class="callout">${esc(plan.scene)}</p><p class="daily-draft">${esc(plan.text)}</p><p class="research-note">文章の基準：${esc(plan.research)}</p><details><summary>画像のALT案を見る</summary><p class="sub">${esc(plan.alt)}</p></details><button class="secondary" data-action="next-daily">別の言い方を見る</button><button class="primary" data-action="save-daily">今日の下書きに保存</button></section><section class="card insight-card"><p class="eyebrow">この案で見たい反応</p><b>${esc(insight.goal)}</b><dl class="insight-list"><div><dt>最初の一言</dt><dd>${esc(insight.hook)}</dd></div><div><dt>画像</dt><dd>${esc(insight.visual)}</dd></div><div><dt>次に比べること</dt><dd>${esc(insight.test)}</dd></div><div><dt>避けること</dt><dd>${esc(insight.avoid)}</dd></div></dl><p class="hint">これは公開傾向から立てた仮説です。実績入力後は、あおば自身の数値で入れ替えます。</p></section><section class="card"><p class="eyebrow">自然に見せるための収集基準</p><ul class="research-list">${writingResearch.map(item=>`<li>${esc(item)}</li>`).join('')}</ul><p class="sub">実在アカウントの文章は転載・模倣せず、観察・長さ・余白の設計だけを抽出しています。あおばはAI生成の架空キャラクターであることを、プロフィールと必要な表示で明示します。</p></section><section class="card"><p class="eyebrow">手動で作る</p><button class="primary" data-go="create">＋ 画像から投稿を作る</button></section>${state.posts.length?state.posts.map((p,i)=>`<article class="card post"><p>${esc(p.text)}</p><small>${esc(p.time)} · 下書き</small><button class="secondary" data-share="${i}">Xで下書きを開く</button></article>`).join(''):`<section class="empty"><b>まだ保存した下書きはありません。</b><p>まずは今日の案を確認してから保存しましょう。</p></section>`}`; }
